#!/usr/bin/env python3
"""
Correlate incidents across DAG, LLM, and Cause & Risk analysis JSON files.

Reads multiple JSON analysis files and creates a consolidated JSON dataset with each incident
showing how it was analyzed across different sources (DAG, GPT-4o-mini, Qwen, Cause & Risk, etc.)
"""
...
class IncidentCorrelator:
    ...
    def load_json_file(self, filepath: str, analysis_type: str):
        ...
        for incident in incidents:
            ...
            # 1. DAG format: single 'analysis' field
            # 2. LLM format: 'summary' + optional 'behavior_analysis'
            # 3. Cause & Risk format: 'cause_analysis' + 'risk_assessment'

            if 'cause_analysis' in incident or 'risk_assessment' in incident:
                # Cause & Risk format: structured with both analyses
                analysis_data = {}
                if 'cause_analysis' in incident:
                    analysis_data['cause_analysis'] = incident['cause_analysis']
                if 'risk_assessment' in incident:
                    analysis_data['risk_assessment'] = incident['risk_assessment']
                self.incidents[incident_id][f'{analysis_type}'] = analysis_data
            elif 'analysis' in incident:
...
def detect_file_type(filename: str) -> str:
    ...
    # Check for Cause & Risk analysis first (new format)
    if '.cause_risk.' in name:
        # Extract model name: my_dataset.cause_risk.gpt-4o-mini.json -> cause_risk_gpt4o_mini
        parts = name.split('.cause_risk.')
        if len(parts) > 1:
            model = parts[1].replace('.json', '').replace('.', '_').replace('-', '_')
            return f'cause_risk_{model}'
        return 'cause_risk'

    # Check for DAG analysis
    elif '.dag.' in name:
        return 'dag'

    # Check for LLM analysis (various model patterns)
    ...
    # Fallback: generic type based on filename
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Correlate incidents across JSON analysis files (DAG, LLM, Cause & Risk)'
    )
    parser.add_argument(
        'files',
        nargs='+',
        help='Input JSON files to correlate (e.g., *.dag.json *.llm.*.json *.cause_risk.*.json)'
    )
    parser.add_argument(
        '-o', '--output',
