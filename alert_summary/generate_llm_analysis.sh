...
# Generate LLM Analysis Script
# Wrapper for alert_dag_parser_llm.py to generate LLM-enhanced analysis from JSONL files
...
Generate LLM-enhanced analysis from JSONL alert file.

Arguments:
    input.jsonl         Input JSONL file (from sample_dataset.py)

Options:
    --output FILE              Output file (default: {input_base}.llm.{model}.json)
    ...
    --behavior-analysis        Generate behavior analysis in addition to summary (requires 2 LLM calls)
    --verbose                  Verbose output
    --help                     Show this help message

Output Files:
    {input_base}.llm.{model}.json   LLM analysis output in JSON format (unless --output specified)

Examples:
    # Generate LLM analysis with default model (gpt-4o-mini)
    $0 my_dataset.jsonl

    # Use Qwen model via Ollama
    $0 my_dataset.jsonl --model qwen2.5:3b --base-url http://10.147.20.102:11434/v1

    # Use Llama3 with custom output name
    $0 my_dataset.jsonl --model llama3 --base-url http://localhost:11434/v1 --output llama3_analysis.txt
    ...
    $0 dataset.jsonl --model llama3 --base-url http://10.147.20.102:11434/v1
EOF
}
...
main() {
    ...
    if [[ -z "$output_file" ]]; then
        ...
        output_file="${base_name}.llm.${model_name}.json"
    fi
    ...
    log_info "Generating LLM-enhanced analysis..."
    ...
    # Build command
    local cmd="python3 $SCRIPT_DIR/alert_dag_parser_llm.py"
    cmd+=" $input_file"
    ...
    [[ "$behavior_analysis" == true ]] && cmd+=" --behavior-analysis"
    ...
    log_success "LLM analysis generated: $output_file"
